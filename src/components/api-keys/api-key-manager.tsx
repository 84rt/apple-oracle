...
export function ApiKeyManager({ onApiKeyUpdate }: ApiKeyManagerProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-6">
        {Object.values(
          Object.entries(MODELS).reduce((groups, [modelKey, config]) => {
            const provider = config.provider
            if (!groups[provider]) groups[provider] = [] as Array<[LLMModel, typeof config]>
            groups[provider].push([modelKey as LLMModel, config])
            return groups
          }, {} as Record<string, Array<[LLMModel, (typeof MODELS)[LLMModel]]>>)
        ).map((group, idx) => {
          const provider = group[0][1].provider
          return (
            <div key={provider + idx} className="space-y-3">
              <h4 className="text-sm font-medium">{provider}</h4>
              <div className="grid gap-4">
                {group.map(([model, config]) => {
